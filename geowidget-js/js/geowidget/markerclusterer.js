  ...
  function addLeftMarkers_() {
    ...
    for (i = 0; i < leftMarkers_.length; ++i) {
      if (isMarkerInViewport_(leftMarkers_[i])) {
        me_.addMarker(leftMarkers_[i], true, null, null, true, null);
      } else {
...
//    GEvent.removeListener(mcfn_);
  ...
  function reAddMarkers_(markers) {
    ...
    for (var i = len - 1; i >= 0; --i) {
      me_.addMarker(markers[i].marker, true, markers[i].isAdded, clusters, true, markers[i].isHidden);
    }
    addLeftMarkers_();
  }
   ...
   */
  this.addMarker = function (marker, opt_isNodraw, opt_isAdded, opt_clusters, opt_isNoCheck, opt_isHidden) {
   ...
   * Highlight a marker.
   *
   * @param {GMarker} marker The marker you want to highlight.
   */
  this.highlightMarker = function (marker) {
  	me_.unhighlightMarker();
	var markerObj;
    for (var i = 0; i < clusters_.length; ++i) {
      if (clusters_[i] && (markerObj = clusters_[i].removeMarker(marker))) {
		clusters_[i].hide();
		map_.addOverlay(marker);
		me_.highlight_ = {
			cluster: clusters_[i],
			marker: marker,
			hidden: markerObj.isHidden
		};
		return true;
      }
    }
  ...
  /**
   * Un-Highlight a marker.
   *
   * @param {GMarker} marker The marker you want to highlight.
   */
  this.unhighlightMarker = function () {
  	if (me_.highlight_) {
		map_.removeOverlay(me_.highlight_.marker);
		me_.highlight_.cluster.show();
		me_.addMarkers([me_.highlight_.marker]);
		if(me_.highlight_.hidden) {
			me_.hideMarker(me_.highlight_.marker);
		}
		delete me_.highlight_;
	}
  };
   ...
   * Hide a marker.
   *
   * @param {GMarker} marker The marker you want to hide.
   */
  this.hideMarker = function (marker, dontRedraw) {
  	if(me_.highlight_ && marker === me_.highlight_.marker) {
		me_.highlight_.hidden = true;
	}
    for (var i = 0; i < clusters_.length; ++i) {
      if (clusters_[i] && clusters_[i].hideMarker(marker)) {
        if(!dontRedraw) clusters_[i].redraw_();
        return;
      }
    }
  };
 ...
 /**
   * Show a marker.
   *
   * @param {GMarker} marker The marker you want to show.
   */
  this.showMarker = function (marker, dontRedraw) {
  	if(me_.highlight_ && marker === me_.highlight_.marker) {
		me_.highlight_.hidden = false;
	}
    for (var i = 0; i < clusters_.length; ++i) {
      if (clusters_[i] && clusters_[i].showMarker(marker)) {
        if(!dontRedraw) clusters_[i].redraw_();
        return;
      }
    }
  ...
  this.getParentCluster = function(marker) {
	...
	/**
	* Show set of clusters
	*/
	this.show = function() {
		hidden_ = false;
		
		var clusters = clusters_;
		for (var i = 0; i < clusters.length; ++i) {
			clusters[i].show();
		}
	};
	...
	*/
	...
	/**
	 * Get whether the set of clusters is hidden.
	 * @return {Boolean}
	 */
...
function Cluster(markerClusterer) {
  ...
  /**
   * Hide a marker in the cluster.
   *
   * @param {GMarker} marker The marker you want to hide.
   * @return {Boolean} Whether find the marker to be hidden.
   */
  this.hideMarker = function (marker) {
    for (var i = 0; i < markers_.length; ++i) {
      if (marker === markers_[i].marker) {
        markers_[i].isHidden = true;
        return true;
      }
    }
    return false;
  ...
  /**
   * Show a marker in the cluster.
   *
   * @param {GMarker} marker The marker you want to show.
   * @return {Boolean} Whether find the marker to be shown.
   */
  this.showMarker = function (marker) {
    for (var i = 0; i < markers_.length; ++i) {
      if (marker === markers_[i].marker) {
        markers_[i].isHidden = false;
        return true;
      }
    }
    return false;
  ...
  this.redraw_ = function (isForce) {
	...
	var realMarkers = this.getRealMarkers();

    if (zoom_ > mz || realMarkers.length <= 1) {
	  ...
	  if (!hidden_) {
	  	var sums = markerClusterer_.getCalculator()(realMarkers);
	  	if (clusterMarker_ === null) {
